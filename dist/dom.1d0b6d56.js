...
window.dom = {
  ...
  create: function create(string) {
    ...
    var container = document.createElement("template"); // 去掉字符串前后的空格 字符串是直接写好的如<div><p><span></span></p></div>,不然就要再创建然后添加
    // 将字符串直接放进template标签会直接变成元素

    container.innerHTML = string.trim(); // return container.content.children[0];
  ...
  attr: function attr(node, name, value) {
    if (arguments.length === 3) {
      node.setAttribute(name, value); // attr(node,name) 获取属性值
  ...
  style: function style(node, name, value) {
    if (arguments.length === 3) {
      ...
      if (_typeof(name) === string) {
        return node.style[name]; // dom.style(div,{color:red})
